...
$(document).ready(function () {
    ...
    document.addEventListener('keydown', function (event) {
        if (event.which >= 48 && event.which <= 57) {
            if (visore == "0") {
                visore = "" + (event.which - 48);
            } else {
                visore.length < visoreMax ? visore = visore + "" + (event.which - 48) : null;
            }
            $("#campo").html(visore);
        } else if (event.which >= 96 && event.which <= 105) {
            if (visore == "0") {
                visore = "" + (event.which - 96);
            } else {
                visore.length < visoreMax ? visore = visore + "" + (event.which - 96) : null;
            }
            $("#campo").html(visore);
        } else if (event.which == 107 || event.which == 171) {
            if (visore.length != 0  && visore.charAt(0)!="-") {
                str += visore;
                str += "+";
                visore = "";
            } else {
                alert("Hai già inserito un operatore!");
            }
    ...
    $(".numero").click(function () {
        visore.length < visoreMax ? visore += this.innerHTML : null;
